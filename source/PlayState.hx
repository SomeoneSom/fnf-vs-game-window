...
class PlayState extends MusicBeatState
{
	...
	var bounceX:Int = 0;
	var bounceY:Int = 0;
	var bounceCounterStep:Float = 0;
	var bounceChangeX:Float = 0;
		...
		if (executeModchart) // dude I hate lua (jkjkjkjk)
			{
				...
				// globals for schmooving

				setVar("appScale", Lib.application.window.scale);
				setVar("displayWidth", Lib.application.window.display.bounds.width);
				...
				Lua_helper.add_callback(lua, "moveWindow", function(x:Int, y:Int, w:Int, h:Int) {
					Lib.application.window.resize(w, h);
				...
				Lua_helper.add_callback(lua, "bounce", function(cx:Float, cy:Float, frames:Int = 3) {
					...
					bounceCount = 4;
					bounceWait = frames;
					bounceCounterStep = 0.5;
					bounceFirstX = Math.floor((Lib.application.window.display.bounds.width-Lib.application.window.width)/2);
		...
		if (executeModchart && lua != null && songStarted)
		{
			...
			if (weBouncin) {
				if (bounceCount > 2 || (bounceCount == 2 && bounceCounterStep == 2)) {
					bounceCount -= bounceCounterStep == 2 ? -1 : 1;
				} else {
					if (bounceCount != 4) {
						bounceCount -= 1;
					}
					if (bounceCount == 0) {
						bounceCount = 0.015625;
					}
					bounceCount *= bounceCounterStep;
					if (bounceCount != 2) {
						bounceCount += 1;
					}
					if (bounceCount == 1.03125 && bounceCounterStep == 0.5) {
						bounceCount = 1;
					}
				}
				if (bounceCount == 1) {
					...
					callLua('realBounce', [bounceChangeX, bounceChangeY, bounceWait, bounceCount, bounceFirstX, bounceFirstY, bounceX, bounceY]);
				}
			}
